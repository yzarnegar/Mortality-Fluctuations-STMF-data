...
p2=ggplot(d1, aes(x = year, y = RTotal,color=Sex)) + 
...
p3=ggplot(d1, aes(x = Week, y = RTotal)) + 
  ...
  theme_minimal()
####  Trend of death rate for group age 15-64 along the years for different countries and sex gropus        
p4=ggplot(d1, aes(x = year, y = R15_64,color=CountryCode)) +
 geom_point(aes(size = Sex)) + xlab('Year')+ylab('Total Death Rate')+
 ggtitle("Trend of total 15-64 age group death rate along years 1990-2020 for different countries and sex groups")+
 theme_minimal()  
...
susa<- summarySE(d3, mvar="RTotal", gvars=c("Year","Sex"))
...
summarySE <- function(data=NULL, mvar, gvars=NULL, na.rm=FALSE,
    ...
    datac <- rename(datas, c("mean" = mvar))
...
nullmodel3 <- lmer( RTotal ~ 1 + (1 + factor(Sex)+Year |CountryCode ), data = d5, REML=FALSE)
#summary(nullmodel1)
#summary(nullmodel2)
#summary(nullmodel3)
...
m1=lmer( RTotal ~ factor(Sex) + (1 + factor(Sex)+Year |CountryCode ), data = d6, REML=FALSE)
out2=summary(m1)
m2=lmer( RTotal ~ factor(Sex)+ Year + (1 + factor(Sex)+Year |CountryCode ), data = d6, REML=FALSE)
out3=summary(m2)
m3=lmer( RTotal ~ factor(Sex)*Year + (1 + factor(Sex)+Year |CountryCode ), data = d6, REML=FALSE)
out4=summary(m3)
...
####################################################################################
####Data anlysis considering all the countries for age group 15-64 before year 2020

d7=subset(mortality , mortality$Year<2020)  
d7$year=d7$Year+d7$Week/52

nullmodel1 <- lmer( R15_64 ~ 1 + (1|CountryCode ), data = d7, REML=FALSE)
nullmodel2 <- lmer( R15_64 ~ 1 + (1 + factor(Sex) |CountryCode ), data = d7, REML=FALSE)
nullmodel3 <- lmer( R15_64~ 1 + (1 + factor(Sex)+Year |CountryCode ), data = d7, REML=FALSE)
#summary(nullmodel1)
#=summary(nullmodel2)
#summary(nullmodel3)
out6=anova (nullmodel1, nullmodel2, nullmodel3)

d8 <- within(d7, Sex<- relevel(Sex, ref = 'f'))
m1=lmer( R15_64 ~ factor(Sex) + (1 + factor(Sex)+Year |CountryCode ), data = d8, REML=FALSE)
out7=summary(m1)
m2=lmer( R15_64 ~ factor(Sex)+ Year + (1 + factor(Sex)+Year |CountryCode ), data = d8, REML=FALSE)
out8=summary(m2)
m3=lmer( R15_64 ~ factor(Sex)*Year + (1 + factor(Sex)+Year |CountryCode ), data = d8, REML=FALSE)
out9=summary(m3)


out10=anova (m1, m2, m3)


sink("/Users/yaldazarnegarnia/result3.txt")
cat("random effect modeing considering all the countries:")
cat("\n")
out6
cat("\n")
out7
